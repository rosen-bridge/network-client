 ...
 */
import type { VoteListItemMetaHash } from './voteListItemMetaHash';
import { bigIntsVoteListItemMetaHash } from './voteListItemMetaHash';
import type { VoteListItemMetaJson } from './voteListItemMetaJson';
import { bigIntsVoteListItemMetaJson } from './voteListItemMetaJson';
import type { VoteListItemMetaUrl } from './voteListItemMetaUrl';
import { bigIntsVoteListItemMetaUrl } from './voteListItemMetaUrl';
import type { VoteListItemProposalType } from './voteListItemProposalType';
import { bigIntsVoteListItemProposalType } from './voteListItemProposalType';
import type { VoteListItemVote } from './voteListItemVote';
import { bigIntsVoteListItemVote } from './voteListItemVote';
import type { VoteListItemVoterRole } from './voteListItemVoterRole';
import { bigIntsVoteListItemVoterRole } from './voteListItemVoterRole';

export type VoteListItem = {
  /** The transaction hash where-in vote was posted */
  vote_tx_hash?: string;
  /** The role of the voter */
  voter_role?: VoteListItemVoterRole;
  ...
  proposal_type?: VoteListItemProposalType;
  ...
  block_time?: number;
  /** Actual Vote casted */
  vote?: VoteListItemVote;
  /** A URL to a JSON payload of metadata */
  meta_url?: VoteListItemMetaUrl;
  /** A hash of the contents of the metadata URL */
  meta_hash?: VoteListItemMetaHash;
  /** The payload as JSON */
  meta_json?: VoteListItemMetaJson;
};

export const bigIntsVoteListItem: Array<string> = [
  ...bigIntsVoteListItemVoterRole.map((item) =>
    item === '' ? 'voter_role' : `voter_role.${item}`,
  ),
  ...bigIntsVoteListItemProposalType.map((item) =>
    item === '' ? 'proposal_type' : `proposal_type.${item}`,
  ),
  ...bigIntsVoteListItemVote.map((item) =>
    item === '' ? 'vote' : `vote.${item}`,
  ),
  ...bigIntsVoteListItemMetaUrl.map((item) =>
    item === '' ? 'meta_url' : `meta_url.${item}`,
  ),
  ...bigIntsVoteListItemMetaHash.map((item) =>
    item === '' ? 'meta_hash' : `meta_hash.${item}`,
  ),
  ...bigIntsVoteListItemMetaJson.map((item) =>
