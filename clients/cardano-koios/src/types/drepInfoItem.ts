...
import type { DrepInfoItemHash } from './drepInfoItemHash';
import { bigIntsDrepInfoItemHash } from './drepInfoItemHash';
import type { DrepInfoItemUrl } from './drepInfoItemUrl';

import { bigIntsDrepInfoItemUrl } from './drepInfoItemUrl';

export type DrepInfoItem = {
  ...
  drep_id?: string;
  /** After which epoch DRep is considered inactive. */
  expires_epoch_no?: DrepInfoItemExpiresEpochNo;
  /** Flag which shows if this credential is a script hash */
  has_script?: boolean;
  /** A hash of the contents of the metadata URL */
  hash?: DrepInfoItemHash;
...
export const bigIntsDrepInfoItem = [
  ...
  ...bigIntsDrepInfoItemHash.map((item) =>
    item === '' ? 'hash' : `hash.${item}`
  ),
  ...bigIntsDrepInfoItemUrl.map((item) =>
    item === '' ? 'url' : `url.${item}`
  ),
];
