...
import { bigIntsTxInfoItemOutputsItemInlineDatum } from './txInfoItemOutputsItemInlineDatum';
import type { TxInfoItemOutputsItemPaymentAddr } from './txInfoItemOutputsItemPaymentAddr';
import { bigIntsTxInfoItemOutputsItemPaymentAddr } from './txInfoItemOutputsItemPaymentAddr';
import type { TxInfoItemOutputsItemReferenceScript } from './txInfoItemOutputsItemReferenceScript';
import { bigIntsTxInfoItemOutputsItemReferenceScript } from './txInfoItemOutputsItemReferenceScript';
import type { TxInfoItemOutputsItemStakeAddr } from './txInfoItemOutputsItemStakeAddr';

import { bigIntsTxInfoItemOutputsItemStakeAddr } from './txInfoItemOutputsItemStakeAddr';

export type TxInfoItemOutputsItem = {
  /**
   * An array of assets on the UTxO
   * @nullable
   */
  asset_list?: TxInfoItemOutputsItemAssetListItem[] | null;
  /** Hash of datum (if any) connected to UTxO */
  datum_hash?: TxInfoItemOutputsItemDatumHash;
  /** Allows datums to be attached to UTxO (CIP-32) */
  inline_datum?: TxInfoItemOutputsItemInlineDatum;
  payment_addr?: TxInfoItemOutputsItemPaymentAddr;
  ...
  stake_addr?: TxInfoItemOutputsItemStakeAddr;
...
export const bigIntsTxInfoItemOutputsItem = [
  ...bigIntsTxInfoItemOutputsItemDatumHash.map((item) =>
    item === '' ? 'datum_hash' : `datum_hash.${item}`
  ),
  ...bigIntsTxInfoItemOutputsItemInlineDatum.map((item) =>
    item === '' ? 'inline_datum' : `inline_datum.${item}`
  ),
  ...bigIntsTxInfoItemOutputsItemPaymentAddr.map((item) =>
  ...
  ...bigIntsTxInfoItemOutputsItemStakeAddr.map((item) =>
