 ...
 */
import type { AddressListItemPaymentCred } from './addressListItemPaymentCred';
import { bigIntsAddressListItemPaymentCred } from './addressListItemPaymentCred';
import type { AddressListItemStakeAddress } from './addressListItemStakeAddress';
import { bigIntsAddressListItemStakeAddress } from './addressListItemStakeAddress';
import type { AddressListItemStakeAddressHex } from './addressListItemStakeAddressHex';
import { bigIntsAddressListItemStakeAddressHex } from './addressListItemStakeAddressHex';

export type AddressListItem = {
  /** A Cardano payment/base address (bech32 encoded) */
  address?: string;
  /** Cardano address in hex format */
  address_hex?: string;
  /** Flag which shows if this credential is a script hash */
  has_script?: boolean;
  /** Payment credential */
  payment_cred?: AddressListItemPaymentCred;
  stake_address?: AddressListItemStakeAddress;
  stake_address_hex?: AddressListItemStakeAddressHex;
};

export const bigIntsAddressListItem: Array<string> = [
  ...bigIntsAddressListItemPaymentCred.map((item) =>
    item === '' ? 'payment_cred' : `payment_cred.${item}`,
  ),
  ...bigIntsAddressListItemStakeAddress.map((item) =>
    item === '' ? 'stake_address' : `stake_address.${item}`,
  ),
  ...bigIntsAddressListItemStakeAddressHex.map((item) =>
    item === '' ? 'stake_address_hex' : `stake_address_hex.${item}`,
  ),
];
